...
import {
  makeStyles, TextField, Button, Modal
...
export default function Uploader(){
  ...
  const handleSearchResult = (text) => {
    const resultList = data.filter(pass=>pass.toLowerCase().indexOf(text)>-1);
          ...
          <div className={`add-new-container ${classes.modalPaper}`}>
            <div className="add-new">
              <form onSubmit={handleAddNew} noValidate autoComplete="off">
                ...
                <input type="file" accept="images/*" name="photo" className={classes.uploadPhoto} id="upload-photo" />
                <TextField
                  name="url"
                  variant="outlined" className={classes.input} type="text" placeholder="Alternative Text" size="small" />
